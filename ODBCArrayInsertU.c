...
static const unsigned int		NumRecInsertedLen = ARRAY_ITEMS(NumRecInserted);
...
void main(int argc, char ** argv)
{
	...
	/* 
	if (argc < 2) {
		fprintf (stderr, "Usage: ODBCArrayInsert username/password@datasource\n\n");
		exit(255);
	}
	*/
...
*/	
	...
	for (i = 0; i < ArraySizeLen; i++) {
		for (j = 0; j < NumRecInsertedLen; j++) {
            ArrayInsert(hdbc, hstmt, TableName, ArraySize[i], NumRecInserted[j]);
	...
	wprintf(L"ODBC ARRAY INSERT TEST COMPLETED.\n\n");
	wprintf(L"===========================================================\n");
...
void ArrayInsert (SQLHANDLE hdbc, SQLHANDLE hstmt, wchar_t *TableName, unsigned int SizeOfArray, unsigned int NumberRecords)
{
	...
	rc = SQLBindParameter(hstmt, 3, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, 6, 0,
	...
	rc = SQLBindParameter(hstmt, 5, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_DECIMAL, 9, 0,
	...
	rc = SQLBindParameter(hstmt, 7, SQL_PARAM_INPUT, SQL_C_SLONG, SQL_INTEGER, 3, 0,
	...
	while (NumRecords > 0) {
		...
		for (i = 0; i < Loop; i++) {
			strcpy(FirstNameArray[i], "Gordon");
			FirstNameIndArray[i] = strlen(FirstNameArray[i]);
			
			strcpy(LastNameArray[i], "Crenshaw");
	...
	wprintf (L"%17d	%10d	", NumberRecords, SizeOfArray);
...
void PrintTableHeader()
{
	wprintf(L"Num Recs Inserted	Array Size	Time		\n");
	wprintf(L"-----------------	----------	------------\n");
}
